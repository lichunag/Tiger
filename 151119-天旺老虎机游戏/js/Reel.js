function Reel(combination,index){
	...
	var i,sy;
	self.reels[0].height = 109;
	self.reels[0].bitmapData.height = self.reels[0].height;
	self.reels[0].bitmapData.setCoordinate(0,109-self.reels[0].height);
	self.reels[2].height = 109;
...
Reel.prototype.wheel = function (){
	...
	if(self.stopFlag && self.currentSpeed <= self.minSpeed && self.indexs[1] == self.combination[self.stopNum][self.index] && self.reels[1].y + self.currentSpeed > 109){
		self.currentSpeed = 109 - self.reels[1].y; 
		self.startReel = false;
...
Reel.prototype.setY = function(){
	...
	if(self.reels[1].y + self.reels[1].height > 327){
		self.reels[1].height = 327 - self.reels[1].y;
		self.reels[1].bitmapData.height = self.reels[1].height;
	}
	if(self.reels[1].y > 109){
		self.reels[0].height = 109;
		self.reels[0].y = self.reels[1].y - 109;
	}else{
	...
	if(self.reels[2].y > 327){
		self.reels[2].visible = false;
	}else if(self.reels[2].y + 109 > 327){
		self.reels[2].height = 327 - self.reels[2].y;
		self.reels[2].bitmapData.height = self.reels[2].height;
	}else{
		self.reels[3].y = self.reels[2].y + self.reels[2].height;
		if(self.reels[3].y < 327){
			self.reels[3].height = 327 - self.reels[3].y;
	...
	if(self.reels[0].y > 0){
		...
		child.bitmapData.setCoordinate(0,109-child.height);
	}
	if(self.reels[3].y >= 327){
		self.reels[3].visible = false;
	}
